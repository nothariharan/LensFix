...
import 'package:lens_fix/widgets/blinking_marker.dart'; // Import New Widget
...
class _HelperHomeScreenState extends State<HelperHomeScreen> {
  ...
  Widget _buildPinDynamic(String status, String category) {
    Color color = status == 'Resolved' ? Colors.greenAccent : Colors.redAccent;
    return Container(
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasData) {
                for (var doc in snapshot.data!.docs) {
                  ...
                  // Extract Urgent Status
                  bool isUrgent = data['isUrgent'] == true;

                  markers.add(
                    Marker(
                      point: LatLng(geo.latitude, geo.longitude),
                      width: isUrgent ? 80 : 50, // Make space for pulse if urgent
                      height: isUrgent ? 80 : 50,
                      child: BlinkingMarker( // <--- WRAP WITH BLINKING MARKER
                        isUrgent: isUrgent,
                        child: _buildPinDynamic(data['status'] ?? 'Pending', data['category'] ?? 'Other'),
                      ),
                    ),
                  );
                }
              ...
              child: Row(
                ...
                children: [
                  ...
                  Transform.translate(
                    ...
                    child: Container(height: 70, width: 70, decoration: BoxDecoration(color: Colors.orangeAccent, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 2), boxShadow: [BoxShadow(color: Colors.orangeAccent.withOpacity(0.4), blurRadius: 20)]), child: IconButton(icon: const Icon(Icons.add_a_photo, color: Colors.black, size: 32), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CameraScreen())))),
                  ),
