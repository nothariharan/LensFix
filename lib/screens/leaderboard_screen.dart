...
class _LeaderboardScreenState extends State<LeaderboardScreen> {
  int _selectedTab = 2; // Default to "All Time"
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance
            .collection('users')
            .orderBy('xp', descending: true) // Sort by Highest XP
            .limit(10)
            .snapshots(),
        builder: (context, snapshot) {
          ...
          String getName(int index) => (getUser(index)['email'] ?? "Unknown").split('@')[0];
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        if (users.length > 1)
                          _buildPodiumItem(
                            rank: 2, name: getName(1), score: getScore(1), 
                            color: Colors.grey[800]!, height: 140, avatarColor: Colors.grey[400]!,
                            rankColor: const Color(0xFFC0C0C0), scoreColor: Colors.greenAccent,
                          ),
                        
                        // Rank 1 (Gold/Winner)
                        if (users.isNotEmpty)
                          _buildPodiumItem(
                            rank: 1, name: getName(0), score: getScore(0), 
                            color: Colors.white, height: 180, avatarColor: Colors.white, 
                            isWinner: true, textColor: Colors.black,
                            rankColor: const Color(0xFFFFD700), scoreColor: Colors.green[800]!,
                          ),
                        
                        // Rank 3 (Bronze)
                        if (users.length > 2)
                          _buildPodiumItem(
                            rank: 3, name: getName(2), score: getScore(2), 
                            color: Colors.grey[900]!, height: 110, avatarColor: Colors.grey[600]!,
                            rankColor: const Color(0xFFCD7F32), scoreColor: Colors.greenAccent,
                          ),
                      ],
                    ),
                  ],
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _buildListItem(rank, getName(dataIndex), "${getScore(dataIndex)} pts");
                  },
                ),
              ),
            ],
  ...
  Widget _buildPodiumItem({
    required int rank, required String name, required String score, 
    required Color color, required double height, required Color avatarColor, 
    bool isWinner = false, Color textColor = Colors.white,
    required Color rankColor, required Color scoreColor,
  }) {
    return Column(
      mainAxisSize: MainAxisSize.min,
      children: [
        Stack(
          ...
          children: [
            Container(
              ...
              child: CircleAvatar(
                radius: 30, backgroundColor: isWinner ? Colors.black : Colors.black54,
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: color, borderRadius: const BorderRadius.vertical(top: Radius.circular(15)),
            border: Border.all(color: isWinner ? const Color(0xFFFFD700) : Colors.white24, width: isWinner ? 2 : 1),
            boxShadow: isWinner ? [BoxShadow(color: const Color(0xFFFFD700).withOpacity(0.4), blurRadius: 25, spreadRadius: 1)] : [],
          ),
          child: FittedBox(
