...
class _HelperTaskScreenState extends State<HelperTaskScreen> {
  ...
  void _showTaskDetails(String docId, Map<String, dynamic> data) {
    Color severityColor = _getSeverityColor(data['severity'] ?? 'low');
        ...
        child: SafeArea(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              ...
              Text("${data['building'] ?? 'Campus'} • ${data['floor'] ?? 'N/A'}", style: TextStyle(color: severityColor, fontWeight: FontWeight.bold)),
            ]),
            ...
            const SizedBox(height: 20),
            Text("SUGGESTED FIX:", style: const TextStyle(color: Colors.grey, fontSize: 10, fontWeight: FontWeight.bold)),
            ...
            SizedBox(width: double.infinity, child: ElevatedButton.icon(
              onPressed: () async {
                ...
                if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Issue Resolved! +100 XP"), backgroundColor: Colors.green));
              },
              style: ElevatedButton.styleFrom(backgroundColor: severityColor, foregroundColor: Colors.black, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15))),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Text("PENDING TASKS", style: GoogleFonts.bebasNeue(letterSpacing: 2, color: Colors.white)), centerTitle: true, backgroundColor: Colors.transparent, leading: IconButton(icon: const Icon(Icons.arrow_back_ios, color: Colors.white), onPressed: () => Navigator.pop(context))),
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.data!.docs.isEmpty) return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: const [Icon(Icons.check_circle_outline, size: 60, color: Colors.grey), SizedBox(height: 10), Text("All Clean! No issues.", style: TextStyle(color: Colors.grey))]));
            ...
            itemBuilder: (context, index) {
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(color: const Color(0xFF1E1E1E), borderRadius: BorderRadius.circular(16), border: Border.all(color: severityColor.withOpacity(0.4))),
                  child: Row(children: [
                    Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: severityColor.withOpacity(0.1), shape: BoxShape.circle), child: Icon(Icons.warning, color: severityColor)),
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Text("${data['building'] ?? 'Campus'} • Floor: ${data['floor'] ?? 'N/A'}", style: TextStyle(color: Colors.grey[600], fontSize: 12)),
                    ])),
